...
print("Length of word", len(images_labels))

#word1_labels = labels[:int(5/6*len(labels))]
word1_labels = labels
print("Length of word labels", len(train_labels))
with open("word1_labels", "wb") as f:
	pickle.dump(word1_labels, f)
del word1_labels'''
######################################################################################################
...
def string_of_images():
	...
	for filename in os.listdir(directory):
		...
		if os.path.isfile(f):
			#pred_probab, pred_class = keras_predict(model, f)
			#im = Image.open(f) 
			#im.show()
			thresh = cv2.imread(f)
			#thresh = cv2.resize(img, (640, 480))
