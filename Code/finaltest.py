...
def recognize():
	...
	while True:
		...
		disc = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(10,10))	#morph. transformations based on image shape
		cv2.filter2D(dst,-1,disc,dst)	# homogeneous blurring
		blur = cv2.GaussianBlur(dst, (11,11), 0)	#filter is a 11x11 matrix
		blur = cv2.medianBlur(blur, 15) # replace each pixel with median of its neighbouring pixels. ideal for salt and pepper noise. 
		thresh = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)[1] #(img_name, thresh_value, max_thresh_val, thresh_type)
		#binary thresholding: 0 or 1 
		#cv2.THRESH_OTSU : thresholding value is not fixed. 
		thresh = cv2.merge((thresh,thresh,thresh))
		thresh = cv2.cvtColor(thresh, cv2.COLOR_BGR2GRAY)
		thresh = thresh[y:y+h, x:x+w]

		(openCV_ver,_,__) = cv2.__version__.split(".")
		#countours is a list of contours in image.
		#each (x,y) in a contour represents boundary points of the object 
		if openCV_ver=='3':
		...
		if len(contours) > 0:
			...
			if cv2.contourArea(contour) > 10000:
				x1, y1, w1, h1 = cv2.boundingRect(contour) #drawing an approximate rectangle around the largest contour
		...
		if count_same_frame > 50:
			...
			if len(seq)>0:
				...
				seq.append(text)
				print(text)
			elif len(seq)==0 and text!='':
				#text = '_'
				seq.append(text)
				print('yes',text)

		# if count_same_frame > 60 and text==' ':
